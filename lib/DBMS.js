...
//
        ...
        try {
            ...
            for (const { type, key, value } of parsed) {
                switch (type) {
                    case 'set': await this.set({key, value}, false); break;
                    case 'delete': await this.delete({key}, false); break;
                    case 'clear': await this.clear(false); break;
                }
            }
        } catch (error) {
    ...
    async set({key, value}, logging = true) {
        ...
        try {
            if (logging) await this.log('set', key, value);
    ...
    async delete({key}, logging = true) {
        ...
        if (this.db.hasOwnProperty(key)) {
            try {
                delete this.db[key];
                if (logging) await this.log('delete', key);
     ...
     *
     */
    async clear(logging = true) {
        try {
            this.db = {};
             if (logging) await this.log('clear');
